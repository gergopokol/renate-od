...
class Ode:
    def __init__(self, coefficient_matrix=numpy.array([0.]), initial_condition=numpy.array([0.])):
        self.coefficient_matrix = coefficient_matrix
        self.initial_condition = initial_condition

    def calculate_solution(self, steps):
        solution = odeint(self.set_up_equation, self.initial_condition, t=steps, args=(self.coefficient_matrix,))
        return solution

    def analytical_solution(self, steps):
        ...
        if self.initial_condition.size == 1:
            ...
            for i in range(steps.size):
                analytical_solution[i, :] = numpy.dot(numpy.dot(numpy.linalg.inv(eigenvectors), self.initial_condition),
                                                      eigenvectors) * numpy.exp(eigenvalues * steps[i])
